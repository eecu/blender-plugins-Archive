...
def draw_fading_label(context, text='', y=100, gap=18, center=True, size=12, color=(1, 1, 1), alpha=1, time=5):
    scale = context.preferences.system.ui_scale * get_prefs().modal_hud_scale
    ...
    if isinstance(text, list):
        ...
        for idx, t in enumerate(text):
            ...
            bpy.ops.machin3.draw_curve_label(text=t, coords=line_coords, center=center, color=line_color, alpha=alpha, time=time + idx)

    else:
        coords = (context.region.width / 2, y)

        bpy.ops.machin3.draw_curve_label(text=text, coords=coords, center=center, color=color, alpha=alpha, time=time)
