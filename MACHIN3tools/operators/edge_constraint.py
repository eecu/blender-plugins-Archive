...
from .. utils.draw import draw_init, draw_point, draw_vector, draw_line, draw_points, draw_lines, draw_label
from .. utils.math import average_locations, create_rotation_matrix_from_vector, dynamic_format, get_loc_matrix, get_face_center
from .. utils.property import step_enum
from .. utils.selection import get_selected_vert_sequences
from .. utils.system import printd
from .. utils.ui import draw_status_item, finish_modal_handlers, finish_status, get_mouse_pos, get_zoom_factor, init_modal_handlers, init_status, scroll, scroll_up

from .. items import ctrl, alt, shift, axis_index_mapping, axis_color_mappings, edge_constrained_transform_mode_items, edge_constrained_transform_axis_items, edge_constrained_mode_items
from .. colors import normal, yellow, green, blue
...
def draw_edge_constrained_transform_status(op):
    def draw(self, context):
        ...
        draw_status_item(row, text="Axis Lock:")

        draw_status_item(row, active=op.is_axis_locking and op.transform_axis=='X', key='X')
        draw_status_item(row, active=op.is_axis_locking and op.transform_axis=='Y', key='Y')
        draw_status_item(row, active=op.is_axis_locking and op.transform_axis=='Z', key='Z')

        draw_status_item(row, active=op.is_axis_locking and op.is_mmb, key='MMB', gap=1)

        if not op.is_zero_scaling:
            draw_status_item(row, key='C', text="Clear Axis Lock" if op.is_axis_locking and not op.is_mmb else "Reset", gap=1)

        if not op.is_zero_scaling:
            draw_status_item(row, active=op.transform_mode=='ROTATE', key='R', text="Rotate", gap=2)
            draw_status_item(row, active=op.transform_mode=='SCALE', key='S', text="Scale", gap=1)

        draw_status_item(row, active=op.is_zero_scaling, key='SHIFT', text="Zero Scale", gap=1)

        if op.transform_mode == 'ROTATE' and not op.is_zero_scaling:
            draw_status_item(row, key='MMB_SCROLL', text="Constrain Mode", prop=op.constrain_mode.title().replace('_', ' '), gap=2)

            draw_status_item(row, active=op.is_snapping, key='CTRL', text="Angle Snap", gap=2)

        if op.transform_mode == 'SCALE' and not op.is_axis_locking and not op.is_zero_scaling:
            draw_status_item(row, active=op.is_direction_locking, key='ALT', text="Direction Lock", gap=2)

        if op.draw_end_align:
            draw_status_item(row, active=not op.end_align, key='E', text="Outer End Alignment", gap=2)

        if op.draw_face_align:
            draw_status_item(row, active=op.face_align, key='F', text="Face Alignment", gap=2)

        if len(op.data) > 1:
            draw_status_item(row, active=op.individual_origins, key='Q', text="Individual Origins", gap=2)

    return draw

class TransformEdgeConstrained(bpy.types.Operator):
    bl_idname = "machin3.transform_edge_constrained"
    bl_label = "MACHIN3: Transform Edge Constrained"
    bl_description = "Transform Edge Constraint"
    ...
    transform_mode: EnumProperty(name='Transform Mode', items=edge_constrained_transform_mode_items, default='ROTATE')
    transform_axis: EnumProperty(name='Transform Axis', items=edge_constrained_transform_axis_items, default='VIEW')
    constrain_mode: EnumProperty(name='Constrain Mode', items=edge_constrained_mode_items, default='DIRECT_PLANE_INTERSECTION')
    ...
    def draw_HUD(self, context):
        if context.area == self.area:
            ...
            dims = draw_label(context, title="Edge Constrained ", coords=Vector((self.HUD_x, self.HUD_y)), center=False)

            action, color = ('Zero Scaling', normal) if self.is_zero_scaling else ('Rotation', blue) if self.transform_mode == 'ROTATE' else ('Scaling', green)
            dims = draw_label(context, title=action, coords=Vector((self.HUD_x + dims.x, self.HUD_y)), center=False, color=color)

            self.offset += 18

            if self.transform_axis == 'VIEW':
                dims = draw_label(context, title="in ", coords=Vector((self.HUD_x, self.HUD_y)), offset=self.offset, center=False, size=10, alpha=0.25)
                draw_label(context, title="View Space", coords=Vector((self.HUD_x + dims.x, self.HUD_y)), offset=self.offset, center=False, alpha=0.5)

            else:
                dims = draw_label(context, title="around " if self.transform_mode == 'ROTATE' else "along ", coords=Vector((self.HUD_x, self.HUD_y)), offset=self.offset, center=False, size=10, alpha=0.25)
                dims += draw_label(context, title="local ", coords=Vector((self.HUD_x + dims.x, self.HUD_y)), offset=self.offset, center=False, alpha=0.5)
                draw_label(context, title=self.transform_axis, coords=Vector((self.HUD_x + dims.x, self.HUD_y)), offset=self.offset, center=False, color=axis_color_mappings[self.transform_axis])

            if self.transform_mode == 'SCALE':
                if self.is_direction_locking:
                    self.offset += 24

                    draw_label(context, title="Direction Locked", coords=Vector((self.HUD_x, self.HUD_y)), offset=self.offset, center=False, color=yellow)

            elif self.transform_mode == 'ROTATE' and not self.is_zero_scaling:
                self.offset += 24

                dims = draw_label(context, title="Mode: ", coords=Vector((self.HUD_x, self.HUD_y)), offset=self.offset, center=False, alpha=0.5)
                draw_label(context, title=self.constrain_mode.title().replace('_', ' '), coords=Vector((self.HUD_x + dims.x, self.HUD_y)), offset=self.offset, center=False)

                self.offset += 18

                dims = draw_label(context, title="Angle: ", coords=Vector((self.HUD_x, self.HUD_y)), offset=self.offset, center=False, alpha=0.5)
                dims += draw_label(context, title=dynamic_format(self.angle, decimal_offset=0 if self.is_snapping else 1), coords=Vector((self.HUD_x + dims.x, self.HUD_y)), offset=self.offset, center=False)

                if self.is_snapping:
                    draw_label(context, title=" Angle Snapping", coords=Vector((self.HUD_x + dims.x, self.HUD_y)), offset=self.offset, center=False, color=yellow)

            if self.draw_end_align and not self.end_align:
                self.offset += 18

                draw_label(context, title="Outer End Aligned", coords=Vector((self.HUD_x, self.HUD_y)), offset=self.offset, center=False, color=green)

            if self.draw_face_align and self.face_align:
                self.offset += 18

                draw_label(context, title="Face Aligned", coords=Vector((self.HUD_x, self.HUD_y)), offset=self.offset, center=False, color=blue)

            if self.individual_origins:
                self.offset += 18

                draw_label(context, title="Individual Origins", coords=Vector((self.HUD_x, self.HUD_y)), offset=self.offset, center=False, color=yellow)
    ...
    def draw_VIEW3D(self, context):
        ...
        if context.area == self.area:
            ...
            if self.is_axis_locking:
                ...
                axis_vector = self.mx.col[axis_index_mapping[axis]].xyz.normalized()
                draw_line([self.origin - axis_vector * 1000, self.origin, self.origin + axis_vector * 1000], color=axis_color_mappings[axis], alpha=0.5)

            for axis in ['X', 'Y', 'Z']:
                axis_vector = self.mx.col[axis_index_mapping[axis]].xyz.normalized()
                draw_line([self.origin + axis_vector * 0.3 * self.zoom_factor, self.origin + axis_vector * self.zoom_factor], color=axis_color_mappings[axis], alpha=0.75)

            if self.is_direction_locking:
                draw_vector(self.scale, origin=self.origin, alpha=0.5, modal=modal)
    ...
    def modal(self, context, event):
        ...
        events = ['MOUSEMOVE', *ctrl, *shift, *alt, 'E', 'R', 'S', 'X', 'Y', 'Z', 'C', 'MIDDLEMOUSE', 'F', 'Q']
        ...
        if event.type in events or scroll(event):
            ...
            elif self.transform_mode == 'ROTATE' and not self.is_zero_scaling and scroll(event):
                if scroll_up(event):
                    self.constrain_mode = step_enum(self.constrain_mode, items=edge_constrained_mode_items, step=-1, loop=True)
    ...
    def invoke(self, context, event):
        ...
        init_status(self, context, func=draw_edge_constrained_transform_status(self))
    ...
    def update_transform_axis(self, context, event):
        if event.type == 'X':
            ...
            if event.value == 'PRESS':
                ...
                mouse_vector = (origin_2d - self.mouse_pos).normalized()

                axes_2d = []

                for axis in ['X', 'Y', 'Z']:

                    axis_vector = self.mx.col[axis_index_mapping[axis]].xyz.normalized() * self.zoom_factor
    ...
    def update_transform_plane(self, context, init=False):
        ...
        def get_origin_dir():
            if self.transform_axis == 'VIEW':
                ...
                axis_vector = self.mx.col[axis_index_mapping[axis]].xyz
                ...
                axis = self.transform_axis[-1]
                origin_dir = self.mx.col[axis_index_mapping[axis]].xyz
