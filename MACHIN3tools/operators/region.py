import bpy
from .. utils.asset import get_asset_details_from_space, get_asset_ids, get_asset_import_method, get_asset_library_reference, get_libref_and_catalog, get_registered_library_references, set_asset_catalog_id, set_asset_import_method, set_asset_library_reference
...
class ToggleVIEW3DRegion(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        if area and get_prefs().toggle_region_warp_mouse_to_asset_border:
    ...
    def get_region_type_from_mouse(self, context, debug=False):
        prefer_left_right = get_prefs().toggle_region_prefer_left_right
        close_range = get_prefs().toggle_region_close_range
        ...
        if screen_name in self.prefs:
            ...
            if libref in get_registered_library_references(context):
                ...
                if catalog_id == '00000000-0000-0000-0000-000000000000':
                    ...
                    set_asset_catalog_id(params, catalog_id)
    ...
    def toggle_region(self, context, areas, regions, region_type='TOOLS', debug=False):
        ...
        toggle_asset_shelf = get_prefs().toggle_region_assetshelf
        toggle_asset_top = get_prefs().toggle_region_assetbrowser_top
        toggle_asset_bottom = get_prefs().toggle_region_assetbrowser_bottom
        ...
        if region_type == 'TOOLS':
            ...
            if region:
                ...
                if (region_overlap and region.width == 1) or (not region_overlap and space.show_region_ui and region.width == 1):
                    ...
                    draw_fading_label(context, text=text, y=100, center=True, size=12, color=red, time=1.2, delay=0.3, cancel='')
...
class ToggleASSETBROWSERRegion(bpy.types.Operator):
    ...
    def get_region_type_from_mouse(self, context, can_close, debug=False):
        close_range = get_prefs().toggle_region_close_range if can_close else 50
